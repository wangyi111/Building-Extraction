...
with tf.Session() as sess:
    ...
    #64*64的图片第一次卷积后还是64*64，第一次池化后变为64*64，第二次卷积后64*64，第三次卷积后64*64，得到80张64*64的特征平面
    ...
    #计算输出 
    prediction=tf.matmul(h_fc1_drop,W_fc2)+b_fc2  #256*1 概率
    #【v2】二值化
    prediction2=tf.round(prediction)  #取距离最近的整数（0.5以下--0；0.5以上--1）  Q：tensor格式中怎样自己设阈值
                                 
    #交叉熵代价函数                    
    cross_entropy=tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=y,logits=prediction))                               
    #Adam下降法优化                            
    train_step=tf.train.MomentumOptimizer(1e-6,0.9).minimize(cross_entropy) 
